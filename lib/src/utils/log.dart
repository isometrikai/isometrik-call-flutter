...
class IsmCallLog {
  ...
  ///- `AppLog.info()` - for info log
  ///- `AppLog.error()` - for error log
  IsmCallLog.success(this.message, [this.stackTrace]) {
    log(
      '\x1B[32m[${IsmCallConstants.name}] - $message\x1B[0m',
      stackTrace: stackTrace,
      name: 'Success',
      level: 500,
    );
  ...
  ///- `AppLog.success()` - for success log
  ///- `AppLog.error()` - for error log
  IsmCallLog.info(this.message, [this.stackTrace]) {
    log(
      '\x1B[33m[${IsmCallConstants.name}] - $message\x1B[0m',
      stackTrace: stackTrace,
      name: 'Info',
      level: 800,
    );
  ...
  ///- `AppLog.success()` - for success log
  ///- `AppLog.error()` - for error log
  IsmCallLog.highlight(this.message, [this.stackTrace]) {
    log(
      '\x1B[36m[${IsmCallConstants.name}] - $message\x1B[0m',
      stackTrace: stackTrace,
      name: 'Highlight',
      level: 400,
    );
